...
def test_startup_greeting_periods():
    assert (
        friday_personality.startup_greeting(datetime(2026, 4, 25, 8, 0)).message
        == "Good morning, Boss. Ready to get things done?"
    )
    assert (
        friday_personality.startup_greeting(datetime(2026, 4, 25, 19, 0)).message
        == "Good evening, Boss. What are we working on today?"
    )
    assert (
        friday_personality.startup_greeting(datetime(2026, 4, 25, 14, 0)).message
        == "Hello Boss."
    )
