...
def get_current_user(
    api_key: str = Security(api_key_header), request: Request = None
) -> dict:
    if not api_key:
        logging.warning(
            f"Auth Attempt: Missing API key. IP: {request.client.host if request else 'Unknown'}"
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="API key missing. Provide an X-API-KEY header.",
    ...
    with _api_key_lock:
        ...
        if client is None:
            logging.warning(
                f"Auth Attempt: Invalid API key used. IP: {request.client.host if request else 'Unknown'}"
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Invalid developer API key.",
